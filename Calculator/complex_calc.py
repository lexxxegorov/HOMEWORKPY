...
def start(update, context):
    context.bot.send_message(update.effective_chat.id, "Ввведите первое комплексное число")
    
    return START 

def input_data_1(update, context):
    global first_number
    first_number = complex (update.message.text)
    print (first_number)
    context.bot.send_message(update.effective_chat.id, "Ввведите второе комплексное число")
  
    return BIO

def input_data_2(update, context):
    global second_number
    second_number = complex (update.message.text)
    print (second_number)
